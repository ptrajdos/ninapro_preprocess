...
def process_mat_obj(
    ...
    emg_np = mat_obj["emg"]
    n_emg_channels = emg_np.shape[1]
    emg_channel_names = [f"C{i}" for i in range(n_emg_channels)]

    force_np = mat_obj["force"]
    n_force_channels = force_np.shape[1]
    force_channel_names = [f"F{i}" for i in range(n_force_channels)]

    combined_np = np.hstack([emg_np, force_np])
    channel_names = emg_channel_names + force_channel_names

    labels = preprocess_labels(mat_obj[lab_key])
    u_labels = np.unique(labels)
    logging.debug(f"Found {len(u_labels)} unique labels: {u_labels}")

    r_signals_list = split_matrix(combined_np, labels, fs=fs, channels=channels, channel_names=channel_names)
...
def run_experiment(
    ...
    mat_files = glob.glob(f"{input_dir}/*_E3_A1.mat", recursive=True)
